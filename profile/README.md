# CordPackers
## About
CordPackers is a Discord desktop client decompilation[^1], patching and repacking project.

Friends of CordPackers (or project that uses CordPackers):
- [Oldcord](https://oldcordapp.com) (I contribute this project to Oldcord for the Oldcord Client, go check it out!)

Check out [CordArchive](https://github.com/cordarchive)!

## Repositories
| Repository | Description |
| :--- | :--- |
| [rePacker](https://github.com/cordpackers/rePacker) | Package up patched app.asar into an installer |
| [reUpdater](https://github.com/cordpackers/reUpdater) | Open source `updater.node` replacement |
| [WumpdleV2](https://github.com/cordpackers/WumpdleV2) | A Sheltupdate fork for repackaged Discord clients |

[^1]: Some code are written in Javascript that can be rewritten easily, however some, such as `.node` addon libraries, are written in Rust and due to Rust being Rust we are not doing a byte match decompilation.
